...
import inspect
                ...
                ### BUSCAR 'How to Perform the Task' y especificar que devuelva el codigo ###
...
When asking for commands or code snippets to perform a task to be used in a console, I want the responses to be wrapped inside <code> tags. This will make it easier to monitor with an external program for my script. Do not wrap URLs or regular text in <code> tags.
...
Then, explain how to perform the task in two sentences, with precise, clear and simple language, if any command is present follow the first instructions you were gaven about wrapping it inside <code> </code> labels. Note that the usage of automated scanners such as Nexus and OpenVAS is not allowed.\n"""
